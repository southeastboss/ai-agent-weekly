...
- **按分区评分分配 `assignArticlesToSections(articles)`**：按 `sourceCategory` 建立分区池 → 入池时过滤 >3 天文章 → 每池内按评分排序 → 仅从本区池取 top quota，不跨区补足；**厂商分区特殊处理**：先去重（每个厂商最多 1 篇），再 Fisher-Yates 随机打乱后取前 3 篇
